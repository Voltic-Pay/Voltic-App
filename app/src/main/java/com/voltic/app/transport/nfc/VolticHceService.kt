...
class VolticHceService : HostApduService() {
    ...
    private fun handleSecondTap(commandApdu: ByteArray): ByteArray {
        ...
        return try {
            ...
            val nonce = BigInteger(parts[0])
            val gasPrice = BigInteger(parts[1])
            Log.d(TAG, "Parsed Nonce: $nonce, GasPrice: $gasPrice")
            ...
            if (NfcSession.useVault) {
                ...
                val signatureHex = chain.signVaultPayment(
                    creds, request.to, request.amountEth ?: "0", nonce, deadline
                )
                
                NfcSession.clear()
                awaitingSecondTap = false
                
                val vaultPayload = "VAULT|$signatureHex|$nonce|$deadline|${request.amountEth ?: "0"}"
            ...
            val rawSignedTx = chain.signEthTransactionOffline(
                creds, request.to, request.amountEth ?: "0", nonce, gasPrice
            )
